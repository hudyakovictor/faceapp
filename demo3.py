...
def normalize_3ddfa_landmarks(landmarks, pose, head_center=None, scale=None):
    """
    Упрощённая нормализация landmarks из 3DDFA.
    Выравнивает по head_center и scale, а также виртуально доворачивает по pose.
    """
    if landmarks is None or len(landmarks) == 0:
        return landmarks

    yaw, pitch, roll = pose
    current_rot = Rotation.from_euler('zyx', [yaw, pitch, roll], degrees=True)
    target_rot = Rotation.from_euler('zyx', [0, 0, 0], degrees=True)
    delta_rot = target_rot * current_rot.inv()

    # Центрируем landmarks по head_center (если задан)
    if head_center is not None:
        landmarks_centered = landmarks - head_center
    else:
        landmarks_centered = landmarks - np.mean(landmarks, axis=0)

    # Вращаем landmarks к целевому положению
    rotated = delta_rot.apply(landmarks_centered)
...
def adapt_block_weights_by_pose(block_scores, delta_pose):
    """
    Понижает вес блоков метрик в зависимости от углов поворота головы (yaw, pitch, roll).
    Чем сильнее отклонение — тем слабее доверие к блоку.
    """
    def get_penalty(delta, sensitive=False):
        yaw, pitch, roll = [abs(d) for d in delta]
        norm = yaw + pitch + roll
        if sensitive:
            if norm > 60: return 0.1
            elif norm > 30: return 0.3
            elif norm > 15: return 0.5
        else:
            if norm > 60: return 0.3
            elif norm > 30: return 0.6
            elif norm > 15: return 0.8
        return 1.0

    penalties = {
        "eyes": get_penalty(delta_pose, sensitive=True),
        "mouth": get_penalty(delta_pose, sensitive=True),
        "nose": get_penalty(delta_pose),
        "symmetry": get_penalty(delta_pose),
        "skull": get_penalty(delta_pose, sensitive=False)
    }

    weighted_scores = {}
    for block, score in block_scores.items():
        if score is not None:
            weighted_scores[block] = round(score * penalties.get(block, 1.0), 5)
        else:
            weighted_scores[block] = None
...
def normalize_mediapipe_landmarks(landmarks, pose, head_center=None, scale=None):
    """
    Нормализация landmarks из MediaPipe по данным pose (yaw, pitch, roll),
    центру головы и масштабу. Использует ось вращения и выравнивает по усреднённому положению.
    """
    ...
    # Центрируем относительно головы, если задано
    if head_center is not None:
        landmarks_centered = landmarks - head_center
    else:
        landmarks_centered = landmarks - np.mean(landmarks, axis=0)
    ...
    # Масштабируем для унификации масштаба
    if scale is not None and scale > 0:
        rotated = rotated / scale
...
def normalize_fan_landmarks(landmarks, pose, target_pose=(0, 0, 0), head_center=None, scale=None):
    """
    Нормализация landmarks из FAN по данным pose (yaw, pitch, roll),
    центру головы и масштабу. FAN использует 68 точек.
    """
    ...
    yaw, pitch, roll = pose
    current_rot = Rotation.from_euler('zyx', [yaw, pitch, roll], degrees=True)
    target_rot = Rotation.from_euler('zyx', list(target_pose), degrees=True)
    delta_rot = target_rot * current_rot.inv()
...
def main(args):
    ...
    for i, param in enumerate(param_lst):
        ...
        if results.multi_face_landmarks:
            ...
            mp_landmarks = normalize_mediapipe_landmarks(mp_raw_landmarks, pose, head_center=t3d, scale=s)
        ...
        if landmarks is None or len(landmarks) == 0:
            ...
            # Нормализация landmarks из 3DDFA (для FAN и универсальных метрик)
            normed_landmarks = normalize_3ddfa_landmarks(landmarks, pose, head_center=t3d, scale=s)

            # Переиспользуем те же нормализованные landmarks и для FAN-метрик
            fan_landmarks = normalize_fan_landmarks(
                landmarks,
                pose=pose,
                head_center=t3d,
                scale=s
        ...
        result = OrderedDict([
            ...
            ("summary", {
                ...
                "block_weights": adapt_block_weights_by_pose(calculate_block_scores(additional_metrics), delta_pose),
                "pose_correction": {
                ...
                "suppressed_blocks": [
                    block for block, weight in adapt_block_weights_by_pose(calculate_block_scores(additional_metrics), delta_pose).items()
            ...
            ("block_score_raw", calculate_block_scores(additional_metrics)),
            ("block_score_adapted", adapt_block_weights_by_pose(calculate_block_scores(additional_metrics), delta_pose)),
